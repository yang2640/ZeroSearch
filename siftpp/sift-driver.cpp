...
main(int argc, char** argv)
{
  ...
  try {
	...
	std::cerr << "has fast approximate math" ;
#else
	std::cerr << "has slow accurate math" ;
#endif
	std::cerr << " (fp datatype is '"
  ...
  while( argc > 0 ) {
	...
	O = std::max
      ...
      if(savegss) {
	verbose && cerr<<"siftpp: saving Gaussian scale space"<<endl ;
	...
	verbose && cerr 
      ...
      if( verbose ) {
        cerr << "siftpp: " ;
	if( ! noorient &   nodescr) cerr << "computing keypoint orientations" ;
	if(   noorient & ! nodescr) cerr << "computing keypoint descriptors" ;
	if( ! noorient & ! nodescr) cerr << "computing orientations and descriptors" ;
	if(   noorient &   nodescr) cerr << "finalizing" ; 
	cerr << endl ;
      }
      ...
      {            
        ...
        /*ofstream out;
		out.open("/dev/output", ios::binary|ios::app|ios::out);
